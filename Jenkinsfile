def projectName = 'aws-dev-spring-boot-container'
...
pipeline {
    ...
    stages{
        stage('Configure CodeBuild') {
            steps{
                sh "aws ecr create-repository --repository-name ${projectName} || true"
                sh """
                ...
                """
                sh "./pipeline/cloudformation_events.sh ${projectName}-init"
            }
        }
